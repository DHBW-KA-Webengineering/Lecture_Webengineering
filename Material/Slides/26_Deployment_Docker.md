...
- _Deployment-Prozess_: Wie kommt die Software von der Entwicklungsumgebung auf das Zielsystem?
...
- `FROM debian:trixie-slim`: Basisimage für das neue Image, hier Debian in der Variante "trixie-slim"
...
- Um Zugriffe von außen (auch Host) zu ermöglichen, wird der Port 80 des Containers auf den Port 8080 des Hosts gemappt 
...
## Alternative: Bun Single-file executables (1)

- `bun build --compile` erzeugt eine ausführbare Datei aus einem JavaScript/TypeScript Projekt
  - Enthält Bun Runtime, Anwendungs-Code, alle Abhängigkeiten
  - Standardmäßig für Compile-Platform (d.h. auf Windows wird eine Windows-exe erzeugt)
  - `--target` für Cross-Compilation
  - Braucht im Ziel-Container-Image keine Bun Runtime mehr
- Dokumentation: [Single-file executable](https://bun.com/docs/bundler/executables)


## Alternative: Bun Single-file executables (2)
